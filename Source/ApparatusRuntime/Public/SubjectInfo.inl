...
inline TOutcome<Paradigm>
FSubjectInfo::SetTraits(Ts&&... Traits)
{
	...
	// Assemble a new fingerprint:
	auto& Fingerprint = GetFingerprintRef();
	const auto FingerprintStatus = Fingerprint.template Add<MakePolite(Paradigm), Ts...>();
	AssessCondition(Paradigm, OK(FingerprintStatus), FingerprintStatus);

	if (!IsNoop(FingerprintStatus))
	{
		// The topology needs change.
		AssessConditionFormat(Paradigm, !IsSolid(), EApparatusStatus::InvalidState,
							  TEXT("Set-adding traits to a subject is not possible during its solid state. "
								   "Do you have an unfinished solid iterating?"));
		...
		AssessOK(Paradigm, UMachine::Instance->MoveSubject<Paradigm>(Id, NewChunk));
	...
	if (Subjective)
	{
		...
		Subjective->DoSetTraitRecords(Traits...);
	}
#endif

	const auto SetTraitsStatus = Chunk->SetTraitsAt(SlotIndex, std::forward<Ts>(Traits)...);
	AssessOK(Paradigm, SetTraitsStatus);

	return SetTraitsStatus;
}

template < EParadigm Paradigm, typename... Ts,
		   TTraitTypesSecurity<Ts...>/*=true*/ >
inline TOutcome<Paradigm>
FSubjectInfo::SetTraits(Ts&&... Traits, const bool bLeaveRedundant)
{
	if (sizeof...(Ts) == 0) // Compile-time branch.
	{
		return EApparatusStatus::Noop;
	...
	if (UNLIKELY(!bLeaveRedundant))
	{
		...
		for (int32 i = 0; i < Fingerprint.TraitsNum(); ++i)
		{
			...
			if (UNLIKELY(!ContainsTrait<Ts...>(TraitType, /*bExact=*/true)))
			{
						  ...
						  const bool           bExact/*=false*/)
{
	...
	auto& Fingerprint = GetFingerprintRef();
	const auto FingerprintStatus = Fingerprint.template RemoveSingle<MakePolite(Paradigm)>(TraitType, bExact);
	AssessCondition(Paradigm, OK(FingerprintStatus), FingerprintStatus);
	...
	if (Subjective != nullptr)
	{
		Subjective->DoRemoveTraitRecord(FingerprintStatus.Payload, /*bExact=*/true);
	}
#endif
...
template < EParadigm Paradigm/*=EParadigm::DefaultInternal*/ >
inline TOutcome<Paradigm>
FSubjectInfo::RemoveTraits(UScriptStruct* const TraitType)
{
	AssessCondition(Paradigm, TraitType != nullptr, EApparatusStatus::NullArgument);
	AssessConditionFormat(Paradigm, IsValid(), EApparatusStatus::InvalidState,
				 		  TEXT("The subject to remove the traits from is invalid. "
							   "Was the subject already despawned?"));
	AssessConditionFormat(Paradigm, !IsSolid(), EApparatusStatus::InvalidState,
						  TEXT("Removing traits from a subject is not possible during a solid state. "
							   "Do you have an unfinished solid iterating?"));
	...
	if (Subjective != nullptr)
	{
		Subjective->DoRemoveTraitRecords(TraitType);
	}
#endif
