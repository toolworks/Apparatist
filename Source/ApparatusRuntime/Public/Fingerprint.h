...
#pragma region Component Operations
  ...
  private:
	...
	struct TComponentOps
	{
		...
		template < EParadigm Paradigm >
		static FORCEINLINE TOutcome<Paradigm>
		RemoveSingle(FFingerprint* const InFingerprint,
	...
	// Trait version.
	template < typename T >
	struct TComponentOps<T, /*bTrait=*/true, false>
	{
		template < EParadigm Paradigm >
		static FORCEINLINE TOutcome<Paradigm>
		Add(FFingerprint* const InFingerprint)
		...
		RemoveSingle(FFingerprint* const InFingerprint,
					 const bool          bExact = false)
		{
			return InFingerprint->template RemoveTrait<Paradigm, T>(bExact);
		}
		...
		Remove(FFingerprint* const InFingerprint)
		{
			return InFingerprint->template RemoveTraits<Paradigm, T>();
		}
	...
	// Detail version.
	template < typename D >
	struct TComponentOps<D, false, /*bDetail=*/true>
	{
		template < EParadigm Paradigm >
		static FORCEINLINE TOutcome<Paradigm>
		Add(FFingerprint* const InFingerprint)
		...
		RemoveSingle(FFingerprint* const InFingerprint,
					 const bool          bExact = false)
		{
			return InFingerprint->template RemoveDetail<Paradigm, D>(bExact);
		}
		...
		Remove(FFingerprint* const InFingerprint)
		{
			return InFingerprint->template RemoveDetails<Paradigm, D>();
		}
	...
	{
		return TComponentOps<C>::template RemoveSingle<Paradigm>(this, bExact);
	}
...
#pragma region Removal
	...
	/**
	 * Remove a single trait type from the fingerprint specification.
	 *
	 ...
	 * @param bExact Should the trait be matched exactly instead of with respect
	 * to its inheritance model.
	 * @returns The actually removed trait type.
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	FORCEINLINE TOutcome<Paradigm, UScriptStruct*>
	RemoveSingle(UScriptStruct* const TraitType,
				 const bool           bExact = false)
	{
		const auto Outcome = Traitmark.template RemoveSingle<MakePolite(Paradigm)>(TraitType, bExact);
		if (LIKELY(!IsNoop(Outcome)))
		{
			HashCache = 0;
		}
	...
	/**
	 * Remove traits based on a type from the fingerprint specification.
	 *
	 * Respects the inheritance model.
	 * 
	 * @tparam Paradigm The paradigm to work under.
	 * @param BaseTraitType The base trait type used for removal.
	 * If @c nullptr, nothing is actually performed.
	 * @returns The actually removed trait type.
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	...
	{
		const auto Outcome = Traitmark.template Remove<MakePolite(Paradigm)>(BaseTraitType);
		if (LIKELY(!IsNoop(Outcome)))
		{
			HashCache = 0;
		}
	 ...
	 * If @c nullptr, nothing is actually performed.
	 * @param bExact Should an exactly matched detail get removed instead of a child one.
	 * @returns The outcome of the operation.
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	FORCEINLINE TOutcome<Paradigm>
	RemoveSingle(const TSubclassOf<UDetail> DetailClass,
				 const bool                 bExact = false)
	{
		const auto Outcome = Detailmark.template RemoveSingle<MakePolite(Paradigm)>(DetailClass, bExact);
		if (LIKELY(!IsNoop(Outcome)))
		{
			HashCache = 0;
		}
	...
	/**
	 * Remove details based on a class from the fingerprint specification.
	 *
	...
	/**
	 * Remove a single trait type from a fingerprint.
	...
	RemoveTrait()
	{
		const auto Outcome = Traitmark.template RemoveSingle<MakePolite(Paradigm), T>();
		if (LIKELY(!IsNoop(Outcome)))
		{
			HashCache = 0;
		}
	...
	/**
	 * Remove traits based on a type from the fingerprint.
	 * Statically-typed version.
	 *
	 * @tparam Paradigm The paradigm to work under.
	 * @tparam T The trait type to remove.
	...
	template < EParadigm Paradigm, class D >
	FORCEINLINE TOutcome<Paradigm>
	RemoveDetail(const bool bExact = false)
	{
		const auto Outcome = Detailmark.template RemoveSingle<MakePolite(Paradigm), D>(bExact);
		if (LIKELY(!IsNoop(Outcome)))
		{
			HashCache = 0;
		}
	...
	RemoveDetail(const bool bExact = false)
	{
		return RemoveDetail<Paradigm, D>(bExact);
	}

	/**
	 * Remove details based on a class from the fingerprint.
	 * Statically-typed default paradigm version.
	 *
	 * @tparam Paradigm The paradigm to work under.
	 * @tparam D The base detail class used for removal.
	 * @returns The outcome of the operation.
	 */
	template < EParadigm Paradigm, class D >
	FORCEINLINE TOutcome<Paradigm>
	...
	RemoveDetails()
	{
		return RemoveDetails<Paradigm, D>();
	}

	/**
	 * Remove individual component(s) from the fingerprint.
	 * 
	 * @tparam Paradigm The paradigm to work under.
	...
	{
		return OutcomeCombine(DoRemoveSingle<Paradigm, Cs>(bExact)...);
	}
	...
	RemoveSingle(const bool bExact = false)
	{
		return RemoveSingle<Paradigm, Cs...>(bExact);
	}
