...
#pragma region Traits Bulk-Setting
	/// @name Traits Bulk-Setting
	/// @{
	 ...
	 * @return The outcome of the operation.
	 */
	template< EParadigm Paradigm = EParadigm::Default, typename... Ts,
			  TTraitTypesSecurity<Ts...> = true >
	TOutcome<Paradigm>
	SetTraits(Ts&&... Traits) const
	{
		const auto Info = FindInfo();
		if (AvoidCondition(Paradigm, Info == nullptr))
		{
		...
		return Info->template SetTraits<Paradigm>(std::forward<Ts>(Traits)...);
	}
	...
	template< EParadigm Paradigm = EParadigm::Default, typename... Ts,
			  TTraitTypesSecurity<Ts...> = true >
	TOutcome<Paradigm>
	SetTraits(Ts&&... Traits, const bool bLeaveRedundant) const
	{
		const auto Info = FindInfo();
		if (AvoidCondition(Paradigm, Info == nullptr))
		{
		...
		return Info->template SetTraits<Paradigm>(std::forward<Ts>(Traits)..., bLeaveRedundant);
	}
	...
	/// @}
#pragma endregion Traits Bulk-Setting
...
#pragma region Trait Removal
	...
	template < EParadigm Paradigm, typename T,
			   more::enable_if_t<AllowsStructuralChanges || IsUnsafe(Paradigm), bool> = true >
	FORCEINLINE TOutcome<Paradigm>
	RemoveTrait(const bool bExact = false) const
	{
		const auto Info = FindInfo();
		AssessCondition(Paradigm, Info != nullptr, EApparatusStatus::InvalidState);
		return Info->template RemoveTrait<Paradigm, T>(bExact);
	}
	 ...
	 *
	 * @tparam Paradigm The security paradigm to use.
	 * @param BaseTraitType The base type for the traits to remove.
	 * If is a @c nullptr, nothing is performed.
	 * @return The outcome of the operation.
	 * @return EApparatusStatus::Noop if there was no such
	 * trait in the first place.
	...
	template < EParadigm Paradigm = EParadigm::Default,
			   more::enable_if_t<AllowsStructuralChanges || IsUnsafe(Paradigm), bool> = true  >
	FORCEINLINE TOutcome<Paradigm>
	RemoveTraits(UScriptStruct* const BaseTraitType) const
	{
		const auto Info = FindInfo();
		AssessCondition(Paradigm, Info != nullptr, EApparatusStatus::InvalidState);
		return Info->template RemoveTraits<Paradigm>(BaseTraitType);
	}
	 ...
	 *
	 * @tparam Paradigm The security paradigm to use.
	 * @tparam T The base type for the traits to remove.
	 * @return The outcome of the operation.
	 */
	template < EParadigm Paradigm, typename T,
			   more::enable_if_t<AllowsStructuralChanges || IsUnsafe(Paradigm), bool> = true >
	...
	RemoveTraits() const
	{
		const auto Info = FindInfo();
		AssessCondition(Paradigm, Info != nullptr, EApparatusStatus::InvalidState);
		return Info->template RemoveTraits<Paradigm, T>();
	}
