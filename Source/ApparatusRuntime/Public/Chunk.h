...
#pragma region Traits Setting
	 ...
	 * @param[in] InTraitData The trait data to set.
	 * @return The status of the operation.
	 */
	EApparatusStatus
	SetTraitAt(const SlotIndexType        SlotIndex,
			   const UScriptStruct* const TraitType,
			   const void* const          InTraitData);

	/**
	 * Swap the trait data for the chunk's subject by its type identifier.
	 *
	 * @param[in] SlotIndex The index of the subject within the chunk.
	 * @param[in] TraitType The type of the trait to swap.
	 * @param[in,out] TraitData The trait data to swap with.
	 * @param[in] bDataInitialized Is the incoming trait data actually initialized.
	 * @return The status of the operation.
	 */
	EApparatusStatus
	SwapTraitAt(const SlotIndexType        SlotIndex,
			    const UScriptStruct* const TraitType,
	 ...
	 * Statically-typed version.
	 *
	 * @tparam T The type of trait to set.
	 * @param SlotIndex The index of the subject within the chunk.
	 * @param Trait The trait to set to.
	 * @return The status of the operation.
	 */
	template < typename T >
	FORCEINLINE EApparatusStatus
	SetTraitAt(const SlotIndexType SlotIndex,
			   const T&            Trait)
	{
		return SetTraitAt(SlotIndex, T::StaticStruct(), static_cast<const void*>(&Trait));
	}

	/**
	 * Set the trait for the chunk's subject by its type identifier.
	 * Statically-typed moving version.
	 * 
	 * This essentially performs a swap.
	 *
	 * @tparam T The type of trait to set.
	 * @param SlotIndex The index of the subject within the chunk.
	 * @param Trait The trait to move.
	 * @return The status of the operation.
	 */
	template < typename T >
	FORCEINLINE EApparatusStatus
	SetTraitAt(const SlotIndexType SlotIndex,
			   T&&                 Trait)
	{
		return SwapTraitAt(SlotIndex, T::StaticStruct(), static_cast<void*>(&Trait));
	}

	/**
	 * Set the trait for the chunk's subject by its type identifier.
	 *
	 * @tparam Ts The types of traits to set.
	 * @param SlotIndex The index of the subject within the chunk.
	...
	EApparatusStatus
	SetTraitsAt(const SlotIndexType SlotIndex,
				Ts&&...             Traits)
	{
		return StatusCombine(SetTraitAt(SlotIndex, std::forward<Ts>(Traits))...);
	}
...
inline EApparatusStatus
UChunk::SetTraitAt(const SlotIndexType        SubjectIndex,
				   const UScriptStruct* const TraitType,
				   const void* const          InTraitData)
{
	check((SubjectIndex != FSubjectInfo::InvalidSlotIndex) &&
...
inline EApparatusStatus
UChunk::SwapTraitAt(const SlotIndexType        SubjectIndex,
					const UScriptStruct* const TraitType,
					void* const                TraitData,
					const bool                 bDataInitialized/*=true*/)
{
	check((SubjectIndex != FSubjectInfo::InvalidSlotIndex) &&
		  (SubjectIndex < Slots.Num()));
	check(TraitType != nullptr);
	check(TraitData != nullptr);

	const auto TraitLineIndex = TraitLineIndexOf(TraitType);
	if (LIKELY((TraitLineIndex != InvalidTraitLineIndex) && (TraitLineTypeAt(TraitLineIndex) == TraitType)))
	{
		return Lines[TraitLineIndex].SwapElementAt(SubjectIndex, TraitData, bDataInitialized);
	}
