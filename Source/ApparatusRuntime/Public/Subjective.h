...
#pragma region Multiple Record Getting

	/**
	 * Get traits data from the records.
	 * Dynamically-typed immutable version.
	 */
	template < typename AllocatorT >
	FORCEINLINE EApparatusStatus
	DoGetTraitRecordsData(UScriptStruct* const             TraitType,
						  TArray<const void*, AllocatorT>& OutTraitsData) const
	{
		OutTraitsData.Reset();
		if (UNLIKELY(TraitType == nullptr))
		{
		...
		{
			if (UNLIKELY(!TraitRecord)) continue;
			if (TraitRecord.GetType()->IsChildOf(TraitType))
			{
				OutTraitsData.Add(TraitRecord.GetData());
			}
		}
		return (OutTraitsData.Num() > 0) ? EApparatusStatus::Success : EApparatusStatus::NoItems;
	}

	/**
	 * Get traits data from the records.
	 * Dynamically-typed mutable version.
	 */
	template < typename AllocatorT >
	FORCEINLINE EApparatusStatus
	DoGetTraitRecordsData(UScriptStruct* const       TraitType,
						  TArray<void*, AllocatorT>& OutTraitsData)
	{
		OutTraitsData.Reset();
		if (UNLIKELY(TraitType == nullptr))
		{
	...
	FORCEINLINE EApparatusStatus
	DoGetTraitRecordsData(TArray<const T*, AllocatorT>& OutTraitsData) const
	{
		OutTraitsData.Reset();
		auto& Traits = GetTraitRecordsRef();
		for (auto& TraitRecord : Traits)
		{
			if (UNLIKELY(!TraitRecord)) continue;
			if (TraitRecord.GetType()->template IsChildOf<T>())
			{
				OutTraitsData.Add(TraitRecord.template GetData<T>());
			}
		}
		return (OutTraitsData.Num() > 0) ? EApparatusStatus::Success : EApparatusStatus::NoItems;
	}
	...
	template < typename T, typename AllocatorT >
	FORCEINLINE EApparatusStatus
	DoGetTraitRecordsData(TArray<T*, AllocatorT>& OutTraitsData)
	{
		OutTraitsData.Reset();
		auto& Traits = GetTraitRecordsRef();
		for (auto& TraitRecord : Traits)
...
#pragma endregion Multiple Record Getting
	...
	FORCEINLINE EApparatusStatus
	DoSetTraitRecords(const Ts&... InTraits)
	{
		return StatusCombine(DoSetTraitRecord(InTraits)...);
	}
						...
						const bool           bExact = false)
	{
		...
		{
			if (UNLIKELY(!Traits[i])) continue;
			if ((Traits[i].GetType() == TraitType)
			 || (!bExact && Traits[i].GetType()->IsChildOf(TraitType)))
			{
				Traits.RemoveAt(i);
				return EApparatusStatus::Success;
			}
	...
	FORCEINLINE EApparatusStatus
	DoRemoveTraitRecords(UScriptStruct* const BaseTraitType)
	{
		...
		{
			if (UNLIKELY(!Traits[i])) continue;
			if (Traits[i].GetType()->IsChildOf(BaseTraitType))
			{
				Traits.RemoveAt(i--);
...
#pragma region Direct Trait Data Access
	 ...
	 * @param bExact Should type be matched exactly instead of hereditary.
				...
				const bool           bExact = false) const
	{
		...
		{
			if (UNLIKELY(!TraitRecord)) continue;
			if (TraitRecord.IsOfType(TraitType, bExact))
			{
	...
	{
		return static_cast<const T*>(GetTraitPtr<Paradigm>(T::StaticStruct(), bExact));
	}
	...
	{
		return static_cast<T*>(GetTraitPtr<Paradigm>(T::StaticStruct(), bExact));
	}
	...
	GetTraitPtr(const bool bExact = false) const
	{
		return GetTraitPtr<Paradigm, T>(bExact);
	}
...
#pragma region Multi-Trait Data Access
				  ...
				  TArray<const void*, AllocatorT>& OutTraits) const
	{
		if (LIKELY(Handle))
		{
			return Handle.template GetTraitsPtrs<Paradigm>(TraitType, OutTraits);
		}
		return DoGetTraitRecordsData(TraitType, OutTraits);
	}
	...
	GetTraitsPtrs(TArray<const T*, AllocatorT>& OutTraits) const
	{
		if (LIKELY(Handle))
		{
			return Handle.template GetTraitsPtrs<Paradigm>(OutTraits);
		}
		return DoGetTraitRecordsData(OutTraits);
	}
	...
	{
		TArray<const void*, AllocatorT> ArrayTemp;
		if (LIKELY(Handle))
		{
			AssessOK(Paradigm, Handle.template GetTraitsPtrs<Paradigm>(TraitType, ArrayTemp));
		}
		else
		{
			AssessOK(Paradigm, DoGetTraitRecordsData(TraitType, ArrayTemp));
		}
		return MakeOutcome<Paradigm, TArray<const void*, AllocatorT>>(MoveTemp(ArrayTemp));
	}
	 ...
	 * @tparam Paradigm The paradigm to work under.
	 * @tparam AllocatorT The type of the allocator used in the results receiver.
	 * @param TraitType The type of traits to get.
	 * @return The resulting list of traits.
	 */
	...
	GetTraitsPtrs(UScriptStruct* const TraitType)
	{
		TArray<void*, AllocatorT> ArrayTemp;
		if (LIKELY(Handle))
		{
			AssessOK(Paradigm, Handle.template GetTraitsPtrs<Paradigm>(TraitType, ArrayTemp));
		}
		...
		return MakeOutcome<Paradigm, TArray<void*, AllocatorT>>(MoveTemp(ArrayTemp));
	}
	...
	GetTraitsPtrs() const
	{
		TArray<const T*, AllocatorT> ArrayTemp;
		if (LIKELY(Handle))
		{
			AssessOK(Paradigm, Handle.template GetTraitsPtrs<Paradigm>(ArrayTemp));
		}
		else
		{
			AssessOK(Paradigm, DoGetTraitRecordsData(ArrayTemp));
		}
		return MakeOutcome<Paradigm, TArray<const T*, AllocatorT>>(MoveTemp(ArrayTemp));
	}
	...
	GetTraitsPtrs()
	{
		...
		return MakeOutcome<Paradigm, TArray<T*, AllocatorT>>(MoveTemp(ArrayTemp));
	}
...
#pragma endregion Multi-Trait Data Access
	...
	/**
	 * Remove traits based on a type from the subject.
	 ...
	 * @param BaseTraitType The base type of the traits to remove.
	...
	template < EParadigm Paradigm = EParadigm::Default >
	FORCEINLINE TOutcome<Paradigm>
	RemoveTraits(UScriptStruct* const BaseTraitType)
	{
		if (LIKELY(Handle))
		{
			return Handle.template RemoveTraits<Paradigm>(BaseTraitType);
		}
		return DoRemoveTraitRecords(BaseTraitType);
	}
	...
	RemoveTraits()
	{
		return RemoveTraits<Paradigm>(T::StaticStruct());
	}
	 ...
	 * 
	 * If there are no such traits in the subject,
	 * nothing is performed and EApparatusStatus::Noop is returned.
	 *
	 * @tparam Paradigm The security paradigm to use.
	 * @tparam T The type of the trait to remove.
...
#pragma region Direct Trait Data Access
	 ...
	 * @param TraitType The type of the trait to get.
	 * Must not be a @c nullptr.
	 * @param bExact Should the type be matched exactly instead of using inheritance.
	 * @return A pointer to the trait's data.
	 * @return nullptr If there is no such trait in the subjective.
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	...
	GetTraitPtr(UScriptStruct* const TraitType,
				const bool           bExact = false) const
	{
		return ISubjective::template GetTraitPtr<MakeUnsafe(Paradigm)>(TraitType, bExact);
	}
	...
	GetTraitPtr(const bool bExact = false) const
	{
		return ISubjective::template GetTraitPtr<MakeUnsafe(Paradigm), T>(bExact);
	}
	...
	GetTraitPtr(const bool bExact = false)
	{
		return ISubjective::GetTraitPtr<MakeUnsafe(Paradigm), T>(bExact);
	}
...
#pragma region Multi-Trait Data Access
	...
	GetTraitsPtrs(UScriptStruct* const             TraitType,
				  TArray<const void*, AllocatorT>& OutTraits) const
	{
		return ISubjective::template GetTraitsPtrs<MakeUnsafe(Paradigm)>(TraitType, OutTraits);
	}
	...
	GetTraitsPtrs(TArray<const T*, AllocatorT>& OutTraits) const
	{
		return ISubjective::template GetTraitsPtrs<MakeUnsafe(Paradigm)>(OutTraits);
	}
	...
	GetTraitsPtrs(UScriptStruct* const TraitType) const
	{
		return ISubjective::template GetTraitsPtrs<MakeUnsafe(Paradigm), AllocatorT>(TraitType);
	}
	...
	GetTraitsPtrs() const
	{
		return ISubjective::template GetTraitsPtrs<MakeUnsafe(Paradigm), T, AllocatorT>();
	}
