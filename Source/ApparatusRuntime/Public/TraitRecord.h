...
#pragma region Typing
	...
	/**
	 * Check if the type of the trait is of a passed-in type.
	 * 
	 * @param InType The type to match against.
	 * @param bExact Should the type be matched exactly
	 * instead of hereditary.
	 * @return The status of examination.
	 */
	FORCEINLINE bool
	IsOfType(UScriptStruct* const InType,
			 const bool           bExact = false) const
	{
		return (Type == InType) || ((!bExact) && Type->IsChildOf(InType));
	}
	...
	template < typename T >
	FORCEINLINE bool
	IsOfType(const bool bExact = false) const
	{
		return IsOfType(T::StaticStruct(), bExact);
	}
...
#pragma region Getting The Data
	...
	GetData() const
	{
		check(Type && Type->template IsChildOf<T>());
		return static_cast<const T*>(DoObtainData());
	}
	...
	GetData()
	{
		...
		bForceEditorPull = true;
#endif
		return static_cast<T*>(DoObtainData());
	}
