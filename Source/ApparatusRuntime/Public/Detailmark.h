...
#pragma endregion Comparison
  ...
  private:
	 ...
	 */
	template < typename C, bool bIsDetail = IsDetailClass<C>() >
	...
	struct TComponentOps<D, /*bIsDetail=*/true>
	{
		...
		{
			return InDetailmark->IndexOf(D::StaticClass(), bExact);
		}
...
#pragma region Search
  ...
  public:
	 ...
	 * @param bExact Should the exact type matching be used instead of inheritance based.
	 * @return The index of the detail class or
	 * @c INDEX_NONE, if there is no such detail within
	 * or @p DetailClass is @c nullptr.
	 */
	int32
	IndexOf(const TSubclassOf<UDetail> DetailClass,
	...
	IndexOf(const bool bExact = false) const
	{
		return TComponentOps<D>::IndexOf(this, bExact);
	}
...
#pragma region Removal
	...
	/**
	 * Remove a single detail class from the detailmark.
	 *'
	 * @tparam Paradigm The paradigm to work under.
	 ...
	 * @returns The exact detail removed.
	 */
	template < EParadigm Paradigm = EParadigm::Default >
	FORCEINLINE TOutcome<Paradigm, TSubclassOf<UDetail>>
	RemoveSingle(const TSubclassOf<UDetail> DetailClass,
				 const bool                 bExact = false)
	{
		if (UNLIKELY(DetailClass == nullptr))
		{
			return MakeOutcome<Paradigm, TSubclassOf<UDetail>>(EApparatusStatus::Noop, TSubclassOf<UDetail>());
		}
		...
		const auto DetailIndex = IndexOf(DetailClass, bExact);
		if (UNLIKELY(DetailIndex == INDEX_NONE))
		{
			// The detail was not actually found:
			return MakeOutcome<Paradigm, TSubclassOf<UDetail>>(EApparatusStatus::Noop, TSubclassOf<UDetail>());
		}

		const auto ActualDetailClass = Details[DetailIndex];
		// We can use swapping here,
		// since fingerprints are never reduced within belts.
		Details.RemoveAtSwap(DetailClass);
		...
		return MakeOutcome<Paradigm, TSubclassOf<UDetail>>(EApparatusStatus::Success, ActualDetailClass);
	}

	/**
	 * Remove all detail classes based on a class from the detailmark.
	 *'
	 * @tparam Paradigm The paradigm to work under.
	 * @param BaseDetailClass The base detail class to search for.
	 * May be a @c nullptr and will ignore be ignored silently
	 * in that case.
	...
	template < EParadigm Paradigm = EParadigm::Default >
	FORCEINLINE TOutcome<Paradigm>
	Remove(const TSubclassOf<UDetail> BaseDetailClass)
	{
		if (UNLIKELY(BaseDetailClass == nullptr))
		{
			return EApparatusStatus::Noop;
		}

		// Check if there is actually such detail in the detailmark...
		const FBitMask& DetailMask = GetDetailMask(BaseDetailClass);
		if (UNLIKELY(!GetDetailsMask().Includes(DetailMask)))
		{
			return EApparatusStatus::Noop;
		}
		...
		{
			if (UNLIKELY(Details[i] == nullptr)) continue;
			if (Details[i]->IsChildOf(BaseDetailClass))
			{
  ...
  private:
	...
	template < EParadigm Paradigm, typename D,
			   TDetailClassSecurity<typename std::remove_pointer<D>::type> = true >
	FORCEINLINE TOutcome<Paradigm>
	DoRemoveSingle(const bool bExact = false)
	{
		return RemoveSingle<Paradigm>(more::flatten<D>::type::StaticClass(), bExact);
	}
	 ...
	 */
	template < EParadigm Paradigm, typename D,
			   TNonDetailClassSecurity<typename std::remove_pointer<D>::type> = true >
	FORCEINLINE TOutcome<Paradigm>
  ...
  public:
	...
	RemoveSingle(const bool bExact = false)
	{
		...
		return OutcomeCombine(DoRemoveSingle<Paradigm, Ds>(bExact)...);
	}
	...
	RemoveSingle(const bool bExact = false)
	{
		return Remove<Paradigm, Ds...>(bExact);
	}
