...
IIterable::Lock() const
{
	...
	if (bInSolid) // Compile-time branch.
	{
		...
		checkf(FormerValue > TNumericLimits<LocksCountValueType>::Min(),
			   TEXT("The solidity overflow detected on the '%s' iterable. "
				    "A possible lockage leaking is detected. Are you iterating the chains up to the end?"),
			   *(CastChecked<UObject>(this)->GetName()));
	}
	else
	{
		...
		checkf(FormerValue >= 0,
			   ...
			   *(CastChecked<UObject>(this)->GetName()));
		checkf(FormerValue < TNumericLimits<LocksCountValueType>::Max(),
			   TEXT("The liquidity overflow detected on the '%s' iterable. "
				    "A possible lockage leaking is detected. Are you iterating the chains up to the end?"),
			   *(CastChecked<UObject>(this)->GetName()));
	}
