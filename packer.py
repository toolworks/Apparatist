...
engine_versions = []
allowed_runtime_platforms = []
allowed_editor_platforms = []
...
try:
    with open(CONFIG_FILENAME) as json_file:
        ...
        if 'AllowedRuntimePlatforms' in config:
            allowed_runtime_platforms = config['AllowedRuntimePlatforms']
        if 'AllowedEditorPlatforms' in config:
            allowed_editor_platforms = config['AllowedEditorPlatforms']
except:
    pass
...
parser.add_argument('--docs', '-d', action='store_const', dest='docs',
...
def pack(engine_version):
    ...
    if args.type == PackageType.GAME:
        if 'EngineVersion' in data:
            del data['EngineVersion']
    else:
    ...
    for module in data['Modules']:
        if args.type == PackageType.ENGINE:
            ...
            if module["Type"] == "Editor":
                platforms = list(allowed_editor_platforms)
            else:
                platforms = list(allowed_runtime_platforms)
            ...
            module['WhitelistPlatforms'] = platforms
        elif args.type == PackageType.GAME and 'WhitelistPlatforms' in module:
            del module['WhitelistPlatforms']
